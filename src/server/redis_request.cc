...
namespace Redis {
size_t Request::proto_inline_max_size_ = 16 * 1024L;
size_t Request::proto_bulk_max_size_ = 512 * 1024L * 1024L;
size_t Request::proto_multi_max_size_ = 10 * 1024 * 1024L;
    ...
    switch (state_) {
      case ArrayLen: {
        ...
        if (line[0] == '*') {
          ...
          if (isOnlyLF || multi_bulk_len_ > (int64_t)Request::proto_multi_max_size_) {
            return Status(Status::NotOK, "Protocol error: invalid multibulk length");
          }
          ...
          if (line.length > Request::proto_inline_max_size_) {
      ...
      case BulkLen: {
        ...
        if (bulk_len_ > Request::proto_bulk_max_size_) {
          return Status(Status::NotOK, "Protocol error: invalid bulk length");
        }
        state_ = BulkData;
        break;
